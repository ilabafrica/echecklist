...
class UserController extends Checklist_Controller_Action
{
  ...
  public function loginAction()
  {
    ...
    else
    {
      ...
      if(empty($this->data['userid'])){
        $this->session->flash = 'Please enter a username!';
        $this->makeDialog($this->data);
        return;
      }
      if(empty($this->data['password'])){
        $this->session->flash = 'Password cannot be blank!';
        $this->makeDialog($this->data);
        return;
      }
  ...
  public function changepwAction()
  {
    ...
    else
    {
      ...
      if(empty($this->data['old_pw'])){
        $this->session->flash = 'Please enter your old password.';
        $this->makeDialog($this->data);
        return;
      }
      if(empty($this->data['password'])){
        $this->session->flash = 'Please enter your new password.';
        $this->makeDialog($this->data);
        return;
      }
      if(empty($this->data['password2'])){
        $this->session->flash = 'Please enter your new password again to confirm.';
        $this->makeDialog($this->data);
        return;
      }
